...
void encryptWithRounds(uint64_t ai, uint32_t* key, int rounds, vector<vector<uint32_t>>& buffers, size_t bufferLimit) {
    ...
    for (int i = 1; i <= rounds; i++) {
        ...
        buffers[i].push_back(v0);
        buffers[i].push_back(v1);

        // Если буфер достиг лимита, записываем его в файл
        if (buffers[i].size() >= bufferLimit) {
...
int main() {
    ...
    // Генерируем 16 случайных байтов (4 * 4 байта для массива uint32_t)
    ...
    vector<vector<uint32_t>> buffers(rounds + 1);
